  ...
  return (
    <>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <div className="group relative h-full bg-gradient-to-br from-slate-900/80 to-dark-bg border border-gray-800/50 rounded-2xl p-6 shadow-lg hover:-translate-y-2 hover:shadow-2xl hover:shadow-accent-blue/20 hover:border-accent-blue/50 transition-all duration-300 cursor-pointer">
                        ...
                        <div className="relative h-full flex flex-col">
                          ...
                          {/* Description */}
                          <p className="text-text-light text-sm leading-relaxed mb-4 opacity-90 flex-grow">
                            {member.description}
                          </p>
                          ...
                          {/* Hint */}
                          <p className="text-xs text-text-light/50 mt-4 text-center">
                            {t.team.hints.clickToFlip} •{" "}
                            {t.team.hints.doubleClickForDetails}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      <div className="group relative h-full bg-gradient-to-br from-dark-bg to-slate-900/80 border border-accent-blue/30 rounded-2xl p-6 shadow-lg cursor-pointer">
                        <div className="absolute inset-0 bg-gradient-to-br from-accent-blue/10 to-accent-green/5 rounded-2xl"></div>
                        <div className="relative h-full flex flex-col justify-center">
                          ...
                          <div className="flex flex-wrap gap-2 justify-center mb-4">
                            {member.techStack
                              ...
                              .map((tech, techIndex) => (
                                <span
                                  key={techIndex}
                                  className="px-2.5 py-1 text-xs font-medium bg-accent-blue/10 border border-accent-blue/30 rounded-lg text-accent-blue"
                                >
                                  {tech}
                                </span>
                              ))}
                          ...
                          {/* Hint */}
                          <p className="text-xs text-text-light/50 mt-auto text-center">
      ...
      {selectedMember !== null && (
        ...
        >
          <div
            className="relative bg-gradient-to-br from-dark-bg to-dark-bg-alt border border-gray-800/50 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close Button */}
            <button
              onClick={closeModal}
              className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-gray-800/50 hover:bg-gray-700/50 text-text-light hover:text-white transition-all duration-200 z-10"
            >
              <svg
                className="w-6 h-6"
                fill="none"
              ...
              {teamMembers[selectedMember] && (
                <div className="grid md:grid-cols-3 gap-6 md:gap-8">
                  ...
                  <div className="md:col-span-1">
                    {teamMembers[selectedMember].photo && (
                      <div className="mb-6 flex justify-center md:justify-start">
                        <div className="relative">
                          <div className="absolute inset-0 bg-gradient-to-r from-accent-green to-accent-blue rounded-full blur-xl opacity-50"></div>
                          <img
                            src={teamMembers[selectedMember].photo}
                            alt={teamMembers[selectedMember].name}
                            className="relative w-40 h-40 rounded-full object-cover border-4 border-accent-blue/50 shadow-2xl"
                          />
                        </div>
                      </div>
                    )}
                    ...
                    <div className="space-y-3">
                      {teamMembers[selectedMember].socialLinks.linkedin && (
                        <a
                          ...
                          className="flex items-center gap-3 px-4 py-2 bg-gray-800/50 hover:bg-gray-700/50 rounded-lg text-text-light hover:text-accent-blue transition-all duration-200"
                        >
                          <svg
                            className="w-5 h-5"
                            fill="currentColor"
                      ...
                      {teamMembers[selectedMember].socialLinks.github && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-3 px-4 py-2 bg-gray-800/50 hover:bg-gray-700/50 rounded-lg text-text-light hover:text-accent-green transition-all duration-200"
                        >
                          <svg
                            className="w-5 h-5"
                            fill="currentColor"
                  ...
                  <div className="md:col-span-2">
                    <h2 className="text-3xl md:text-4xl font-bold text-accent-green mb-2">
                    ...
                    <div className="flex flex-wrap gap-2 mt-6">
                      {teamMembers[selectedMember].techStack.map(
                        (tech, techIndex) => (
                          <span
                            key={techIndex}
                            className="px-3 py-1.5 text-sm font-medium bg-dark-bg-alt/80 border border-gray-700/50 rounded-lg text-text-light hover:border-accent-green/50 hover:text-accent-green transition-all duration-200"
                          >
                            {tech}
                          </span>
                        )
